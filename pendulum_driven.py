...
class InputGenerator():
    ...
    def get_input(self,t):
        input = torch.sum(self.a * torch.sin(self.w*t))
        return input

# def input_generator(t, w, a):
#     input = 1 * torch.sin(t * 6) + 2.5 * torch.sin(8.5 * t) + 2 * torch.sin(11.7 * t)
#     return input

# define pendulum dynamics
# to make conservative we set b=0 (default)
class pendSys(nn.Module):
    def __init__(self, input_generator, m=0.5, l=0.3, J=0.1, b=0.0, g=9.8):
    ...
    def set_input_generator(self,input_generator):
        self.input_generator= input_generator

    def forward(self, t, x):
        input = self.input_generator.get_input(t)
        dHdx = torch.empty(2, 1)
        dHdx[0] = -self.m*self.g*self.l*torch.sin(x[0]) + input
...
w = torch.tensor([6, 8.5, 11.7])
a = torch.tensor([1, 2.5, 2])

input_generator0 = InputGenerator(w,a)

with torch.no_grad():
    true_x0 = torch.Tensor([[2.0], [0.0]])
    t = torch.linspace(0.0, run_time, data_size)
    true_x = odeint(pendSys(input_generator0), true_x0, t, method='dopri5')
...
input_generators = []
num_freqs = 4
frequencies = 20*torch.rand(num_data_sets,num_freqs)
amplitudes = 3*torch.rand(num_data_sets, num_freqs)

for i in range(num_data_sets):
    input_generators.append(InputGenerator(frequencies[i,:],amplitudes[i,:]))
    set_x0 = torch.Tensor([[np.random.uniform(low=-1.5, high=1.5)], [0.0]])
    set_x = odeint(pendSys(input_generators[i]), true_x0, t, method='dopri5')
...
class PHS_Func(nn.Module):
    ...
    def forward(self, t, x):
        ...
        m = self.Mnet(x[0].unsqueeze(1)) + min_mass
        dx = torch.empty(2, 1)
        dx[0] = x[1]/m  # q dot
        dx[1] = -dVdx[0]+input
...
model = PHS_Func(input_generator0)
...
for epoch in range(epochs):
    ...
    for set_ind in set_order:
        ...
        model.set_input_generator(input_generators[set_ind])
